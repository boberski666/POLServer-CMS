...
class AutoLoader {
    ...
    protected $classDirs = array('core/', '../core/', 'plugins/');
    protected $skippedClasses = array('AutoLoader', 'Config');
    protected $cacheFileDir = 'cache/';
    protected $map = array();

    private function __construct() {
        $this->cacheFileDir = ROOT_ADMIN_DIR . $this->cacheFileDir;
        $this->getMap();
    ...
    protected function generateMap() {
        foreach ($this->classDirs as $path) {
            $this->readDir(ROOT_ADMIN_DIR . $path);
        }
        $this->saveMap();
    }
        ...
        if ($dir != false) {
            while (($file = readdir($dir)) !== false) {
                if (is_file($path . $file) && strstr($file, self::FileSufix)) {
                    $className = str_replace(self::FileSufix, '', $file);

                    if (!isset($this->map[$className]))
                        $this->map[$className] = $path . $file;
