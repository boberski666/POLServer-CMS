...
use RedBeanPHP\Plugin\CustomPDO as CustomPDO;
use RedBeanPHP\Plugin\CustomPDOStatement as CustomPDOStatement;
...
class RPDO implements Driver
{
	...
	public function connect()
	{
		...
		try {
			...
			$this->pdo = new CustomPDO( $this->dsn, $user, $pass );
			$this->setEncoding();
...
class CustomPDO extends \PDO {
	...
	public static $caller_object;

	public function __construct($dsn, $username = NULL, $password = NULL, $atrr = NULL) {
		parent::__construct($dsn, $username, $password, $atrr);
		$this->setAttribute(\PDO::ATTR_STATEMENT_CLASS, array('RedBeanPHP\Plugin\CustomPDOStatement', array($this)));
	}

	public function query($query) {
		if (DEBUG) {
			$start = microtime(true);
			$check_result = (bool) ($result = parent::query($query));
			$time = microtime(true) - $start;
			$error = parent::errorInfo();
			$trace = debug_backtrace();
			CustomPDO::$caller_object = get_class($trace[1]['object']) . $trace[1]['type'] . $trace[1]['function'] . '()';
			$file = $trace[0]['file'];
			$line = $trace[0]['line'];
			unset($trace);
		}else
		...
		if (is_int(stripos($query, 'SELECT ')) && stripos($query, 'SELECT ') < 5)
			$type = 'query';
		elseif (is_int(stripos($query, 'UPDATE ')) && stripos($query, 'UPDATE ') < 5)
			$type = 'modify';
		elseif (is_int(stripos($query, 'INSERT ')) && stripos($query, 'INSERT ') < 5)
			$type = 'add';
		elseif (is_int(stripos($query, 'DELETE ')) && stripos($query, 'DELETE ') < 5)
			$type = 'delete';
	...
	public function exec($query) {
		if (DEBUG) {
			$start = microtime(true);
			$check_result = (bool) ($result = parent::exec($query));
			$time = microtime(true) - $start;
			$error = parent::errorInfo();
			$trace = debug_backtrace();
			CustomPDO::$caller_object = get_class($trace[1]['object']) . $trace[1]['type'] . $trace[1]['function'] . '()';
		...
		if (DEBUG)
			CustomPDO::$log[] = array(
				...
				'line' => $line,
				'file' => $file,
				'object' => CustomPDO::getCallerObject(),
				'type' => $type,
				'result' => $check_result,
	...
	public function beginTransaction() {
		if (DEBUG) {
			$start = microtime(true);
			$check_result = parent::beginTransaction();
			$time = microtime(true) - $start;
			$error = parent::errorInfo();
			$trace = debug_backtrace();
			CustomPDO::$caller_object = get_class($trace[1]['object']) . $trace[1]['type'] . $trace[1]['function'] . '()';
	...
	public static function getLog() {
		...
		if (DEBUG) {
			...
			if (self::$debuged_file_name != 'index.php' && self::$debuged_file_name != 'ajax_debuger.php' && $log_array['totalQueries'][0])
				$_SESSION['ajax_debug_array'][] = $log_array;
		}
...
class CustomPDOStatement extends \PDOStatement {
	...
	public function execute($input_parameters = null) {
		if (DEBUG) {
			$start = microtime(true);
			$check_result = (bool) ($result = parent::execute());
			$time = microtime(true) - $start;
			$error = parent::errorInfo();
			$trace = debug_backtrace();
			CustomPDO::$caller_object = get_class($trace[1]['object']) . $trace[1]['type'] . $trace[1]['function'] . '()';
		...
		if (is_int(stripos($this->getSQL(), 'SELECT ')) && stripos($this->getSQL(), 'SELECT ') < 5)
			$type = 'query';
		elseif (is_int(stripos($this->getSQL(), 'UPDATE ')) && stripos($this->getSQL(), 'UPDATE ') < 5)
			$type = 'modify';
		elseif (is_int(stripos($this->getSQL(), 'INSERT ')) && stripos($this->getSQL(), 'INSERT ') < 5)
			$type = 'add';
		elseif (is_int(stripos($this->getSQL(), 'DELETE ')) && stripos($this->getSQL(), 'DELETE ') < 5)
			$type = 'delete';
	...
	public function bindParam($paramno, &$param, $type = \PDO::PARAM_STR, $maxlen = null, $driverdata = null) {
		$this->bound_params[$paramno] = array(
			'value' => &$param,
			'type' => $type,
			'maxlen' => (is_null($maxlen)) ? self::NO_MAX_LENGTH : $maxlen // ignore driver data
		);
